...
with open("cbc_plaintext.txt", "r",encoding="UTF-8") as doc, open("cbc_ciphertext.txt", "w",encoding="UTF-8") as kq:  # chon encoding la utf-8
    a = 0
    block = []
    ...
    while True:
        ...
        if a == 8:                                  # neu block da du 8byte = 64 bit, tien hanh ma hoa
            for i in range(8):
                block[i] = block[i] ^ sub_block[i]  # xor khoi text voi gia tri cipher truoc do                             
